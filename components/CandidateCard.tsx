...
type Candidate = {
  ...
  price: number;
  score: number;
  subscores: { pole: number; flag: number; context: number };
    ...
    <div className="card-interactive bg-bg-card border border-terminal-gray-dim/40 rounded-sm p-4">
      ...
      <div className="grid grid-cols-4 gap-2 mb-3 text-xs">
        <div>
          ...
          <div className="font-mono text-text tabular-nums">
            +{polePct}%
            <span className="text-text-muted"> / {c.pattern.poleDays}d</span>
          </div>
        </div>
        <div>
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-terminal-gray-dim/30">
          <div className="text-[10px] text-terminal-gray font-mono uppercase tracking-widest mb-2">Score breakdown</div>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] font-mono text-text-dim">
            ...
            <div>10-EMA: <span className="text-text">${c.ema.ema10?.toFixed(2) ?? '—'}</span></div>
